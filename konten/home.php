<?php
    ...
    if($_SESSION['menu']=='MANAJEMEN'){
      $sql="SELECT SUM(biaya.jumlah_biaya-tagihan.total_berbayar-tagihan.potongan) AS total_tunggakan FROM tagihan,siswa,biaya,periode WHERE tagihan.dihapus_pada IS NULL AND tagihan.id_siswa=siswa.id_siswa AND tagihan.id_biaya=biaya.id_biaya AND biaya.id_periode=periode.id_periode AND biaya.tanggal_jatuh_tempo<'$tanggal_jatuh_tempo'";
      }else {
       $id_siswa=$_SESSION['id'];
