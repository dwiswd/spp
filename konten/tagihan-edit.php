            ...
            <div class="card">
                ...
                <div class="card-body">
                    <form action="aksi/tagihan.php" method="post">
                        ...
                        <input type="date" name="tanggal_jatuh_tempo" class="form-control" <?php if ($tagihan['total_berbayar']>0) { ?> readonly <?php } ?> value="<?= $tagihan ['tanggal_jatuh_tempo']; ?>">

                       
                        <div class="row">
                            <div class="col-md-4">
                                <label for="siswa">Total Tagihan</label>
                                <input type="number" name="jumlah_biaya" class="form-control text-right" <?php if ($tagihan['total_berbayar']>0) { ?> readonly <?php } ?> value="<?= ($tagihan  ['jumlah_biaya']); ?>">
                       

                            </div>
                            <div class="col-md-4">
                                <label for="siswa">Terbayar</label>
                                <input type="text" class="form-control text-right" <?php
                             if ($tagihan['total_berbayar'] > 0) { ?>  <?php } ?> readonly value="<?= $tagihan['total_berbayar']; ?>">

                            </div>
                            <div class="col-md-4">
                                <label for="siswa">Sisa Tagihan</label>
                                <input type="text" class="form-control text-right" readonly value="<?= number_format($tagihan['jumlah_biaya'] - $tagihan['total_berbayar']); ?>">

                            </div>
                        </div>
                        <?php
                        if ($tagihan['total_berbayar'] > 0) {
                        ?>
