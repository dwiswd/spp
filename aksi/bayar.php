...
if ($_POST) {
    if ($_POST['aksi'] == 'tambah') { //Untuk Manajemen
        ...
        $sql = "INSERT INTO bayar (id_bayar,id_siswa,id_bayar_metode,keterangan,bukti,status_vertifikasi,nominal_bayar,tanggal_bayar,dibuat_pada,diubah_pada,dihapus_pada) VALUE(DEFAULT,$id_siswa,$id_bayar_metode,'$keterangan','$bukti','$status_vertifikasi',$nominal_bayar,'$tanggal_bayar',DEFAULT,DEFAULT,DEFAULT)";
        ...
        //echo $id_bayar;
        $sql_tagihan = "SELECT tagihan.*,biaya.deskripsi_biaya,biaya.jumlah_biaya,biaya.tanggal_jatuh_tempo FROM tagihan,biaya WHERE tagihan.id_biaya=biaya.id_biaya AND tagihan.total_berbayar<(biaya.jumlah_biaya-tagihan.potongan) AND tagihan.id_siswa=$id_siswa ORDER BY biaya.tanggal_jatuh_tempo ASC";
        //echo $sql_tagihan;
        ...
        while ($tagihan = mysqli_fetch_array($query_tagihan)) {
            ...
            $total_berbayar = $tagihan['total_berbayar'];
            if ($alokasi_dana > ($jumlah_biaya - $total_berbayar - $potongan)) {
                $dibayarkan = $jumlah_biaya - $total_berbayar - $potongan;
            } else {
                $dibayarkan = $alokasi_dana;
            }
            //Insert table Bayar_alokasi
            $sql_alokasi = "INSERT INTO bayar_alokasi(id_bayar_alokasi,id_bayar,id_tagihan,total_alokasi,dialokasikan_oleh,dibuat_pada,diubah_pada,dihapus_pada) VALUE(DEFAULT,$id_bayar,$id_tagihan,$dibayarkan,'Otomatis Oleh Sistem',DEFAULT,DEFAULT,DEFAULT)";
            mysqli_query($koneksi, $sql_alokasi);
            //Update Tagihan 
            $sql_update_tagihan = "UPDATE tagihan SET total_berbayar=total_berbayar+$dibayarkan WHERE id_tagihan=$id_tagihan";
        ...
        //Ubah Status Vertifikasi Pembayaran(Jika Yang Membayar adalah petugas maka status sudah vertifikasi namun jika yang membayar adalah siswa/ortu siswa maka status belum vertifikasi)
        $sql_ver = "UPDATE bayar SET status_vertifikasi='sudah vertifikasi' WHERE id_bayar=$id_bayar";
...
if ($_GET) {
    if ($_GET['aksi'] == 'hapus') {
        ...
        while ($row3 = mysqli_fetch_array($query3)) {
            ...
            $sql4 = "UPDATE tagihan SET total_berbayar=total_berbayar-$total_alokasi WHERE id_tagihan=$id_tagihan";
